...
if ($fileExists) {
    ...
    ForEach ($file in (Get-ChildItem "C:\users\Public\Desktop\" -Include "Tableau*2021*.lnk" -Recurse) ) {
        Remove-Item $file.FullName
    }
  ForEach ($file in (Get-ChildItem "C:\users\Public\Desktop\" -Include "Tableau Prep*2021*.lnk" -Recurse) ) {
        Remove-Item $file.FullName
    }
...
<#$UdfContent
#We need the folder names since, in package cache, the folder names are all as shown below
$oldPrepRemovals = Get-ChildItem -Recurse -Path "C:\programdata\Package Cache\{b24054df-9bab-409c-a91e-93a62b9cbc6c}" -Include "Tableau*.exe"
try{
start-process $oldPrepRemovals.FullName -ArgumentList "/uninstall /quiet"
} catch{
  Write-host "Error uninstalling old Prep"
}

try{
$oldPrepRemovals = Get-ChildItem -Recurse -Path "C:\programdata\Package Cache\{633037c7e-a10f-42ab-a64c-85ea5bfc1c11}" -Include "Tableau-setup-p*$Year*.exe"  -exclude "C:\users\%username%\appdata\Roaming\think-cell"
start-process $oldPrepRemovals.FullName -ArgumentList "/uninstall /quiet"
} catch {
Write-host "Error uninstalling old Prep"
}
